...
// Volume control
//#define POT_VOL_ANALOG_IN 34      // Pin that will connect to the middle pin of the potentiometer.
...
float Volume1;                               // Volume WAV 1
float Volume2;                               // Volume WAV 2
float VolumeM;                               // Master Volume
float Speed1;                                // Speed WAV 1
...
bool LoadWavFileHeader(String FileName, Wav_Struct *Wav)
{
  ...
  else
  {
    ...
    if (ValidWavData(&WavHeader))
    {
      ...
      Wav->BytesPerSample = WavHeader.BitsPerSample / 8;
      return true;
    }
    else
      return false;
...
void ReadFile(Wav_Struct *Wav)
{
  ...
  ///*********************************************************************************************************************
  // Serial.print("Wav->TotalBytesRead \t");
  // Serial.println(Wav->TotalBytesRead);      // TESTING
...
bool InitWavFiles()
{
  ...
  if (LoadWavFileHeader("/eng1.wav", &Wav1))
    return LoadWavFileHeader("/alert1.wav", &Wav2); // only bother trying to load this if first loads ok
  else
    return false;
}

void Interpolate(Wav_Struct &Wav)
{
  ...
  float CopyOfSpeed = Wav.Speed; // Copy into this var as it could potentially change during this routine giving odd results
                                 // increase the counter, if it goes to a new integer digit then write to DAC

  ActualIncreaseBy = Wav.IncreaseBy; // default if not playing slower than normal
  if (CopyOfSpeed <= 1.0)            // manipulate IncreaseBy
    ActualIncreaseBy = Wav.IncreaseBy * (abs(CopyOfSpeed));
  Wav.Count += ActualIncreaseBy;
  IntPartOfCount = floor(Wav.Count);

  //-----------------
  *Left = (Wav.Samples[Wav.DataBuffIdx + 1] << 8) + Wav.Samples[Wav.DataBuffIdx];      // Get last left channel Data
  *Right = (Wav.Samples[Wav.DataBuffIdx + 3] << 8) + Wav.Samples[Wav.DataBuffIdx + 2]; // Get last right channel Data

  // DataBuffIdx += BytesPerSample;
  // DataIdx += BytesPerSample;
  ...
  if (IntPartOfCount > Wav.LastIntCount)
  {
    if (CopyOfSpeed > 1.0)
    {
      ...
      Wav.DataBuffIdx += Wav.BytesPerSample * int(IntPartAsFloat); // always increase by the integer part
      Wav.DataIdx += Wav.BytesPerSample * int(IntPartAsFloat);     // always increase by the integer part
      Wav.SpeedUpCount += DecimalPart;                             // This keeps track of the decimal part
      ...
      if (Wav.SpeedUpCount >= 1)
      {
        Wav.DataBuffIdx += Wav.BytesPerSample; // move another pos into data
        Wav.DataIdx += Wav.BytesPerSample;     // move another pos into data
        Wav.SpeedUpCount--;                    // Take it off SpeedUpCount
      }
    }
    // gone to a new integer of count, we need to send a new value to the I2S DAC next time
    // update the DataIDx counter

    Wav.LastIntCount = IntPartOfCount;
    Wav.DataBuffIdx += Wav.BytesPerSample; // 4, because 2 x 16bit samples
    Wav.DataIdx += Wav.BytesPerSample;     // 4, because 2 x 16bit samples
    Wav.TimeElapsed = 1000 * Wav.DataIdx / Wav.BytesPerSample;
    Wav.TimeLeft = Wav.PlayingTime - Wav.TimeElapsed;
    if (Wav.DataIdx >= Wav.DataSize) // end of data, flag end
    {
      Wav.Count = 0;               // reset frequency counter
      Wav.DataIdx = Wav.DataStart; // reset data pointer back to beginning of WAV data
      Wav.Playing = false;         // mark as completed
      Wav.TimeLeft = 0;
    }
    else if (Wav.DataBuffIdx >= NUM_BYTES_TO_READ_FROM_FILE)
      FillFileBufer();
  }

  // if (LastNumBytesRead - DataBuffIdx < 4) // if less than 4 bytes left in buffer, set index to last unread byte
  // {											 // then re-fill from file into Samples buffer from there
  // 											 // Serial.print(LastNumBytesRead);
  // 											 // Serial.print("\t");
  // 											 // Serial.print(DataBuffIdx);
  // 											 // Serial.print("\t");
  // 											 // Serial.println(LastNumBytesRead - DataBuffIdx);
  // 											 // uint16_t bytesLeft = LastNumBytesRead - DataBuffIdx;
  // 											 // TotalBytesRead -= bytesLeft;
  // 											 // DataIdx = TotalBytesRead;
  // 											 // WavFile.seek(44 + DataIdx);
  // 											 // FillFileBufer();
  // }
}
...
uint16_t MixWavs(byte *Samples)
{
  ...
  while ((Wav1Idx < Wav1.LastNumBytesRead) | (Wav2Idx < Wav2.LastNumBytesRead))
  {
    ...
    if (Wav1.Playing)
    {
      ...
      Sample_Wav1 = *((int16_t *)(Wav1.Samples + Wav1Idx)) * Volume1;
    }

    if (Wav2.Playing)
      Sample_Wav2 = *((int16_t *)(Wav2.Samples + Wav2Idx)) * Volume2;

    Sample = Sample_Wav1 + Sample_Wav2; // Mix both sounds with their individual volume setting
  ...
  { // We step 2 bytes at a time as we're using 16bits per channel

    // Serial.print("Samples \t");      //*******************************
    // Serial.println(*((int16_t *)(Samples+i)));

    *((int16_t *)(Samples + i)) = (*((int16_t *)(Samples + i))) * VolumeM;
  }
...
void PlayWavs()
{
  ...
  Volume1 = float(analogRead(POT_VOL_WAV1_ANALOG_IN)) / 2047;   // Volume for Wav file 1
  Volume2 = float(analogRead(POT_VOL_WAV2_ANALOG_IN)) / 2047;   // Volume for Wav file 2
  VolumeM = float(analogRead(POT_SPEED_WAV1_ANALOG_IN)) / 2047; // Speed for Wav file 1
...
void setup()
{
  ...
  if (InitWavFiles() == false)
    while (true)
      ;                // If a problem terminate program
