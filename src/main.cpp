...
uint16_t MixWavs(byte* Samples)
{
  ...
  while((Wav1Idx<Wav1.LastNumBytesRead)|(Wav2Idx<Wav2.LastNumBytesRead))
  {
    ...
    Sample_Wav2=0;
    if(Wav1.Playing)
      float Wav1_y0, Wav1_y1;                                   // prev and next sample for interpolation
...
// double waveform_val(byte* Samples, int i)
// {
// return *(Samples->samples[i]);
// }

// double waveform_ival(byte* Samples, double i)
// {
// int x0 = (int)i;
// int x1 = (int)(i+1.0);
// double f0, f1;
// if (x1>=waveform_size(Samples)) x1 = 0;
// f0 = waveform_val(Samples,x0);
// f1 = waveform_val(Samples,x1);
// return f0 + (f1 - f0)*(i-x0);
// }

// // Linear Interpolation
// float LinearInterpol(float y0, float y1, float currentX)
// {
...
//   interpolated_value=(y0*(1-scaleValue))+(y1*scaleValue);
